Shader "MIRAI/CorePP"
{
    Properties
    {
        _ScreenDimensions("Screen Dimensions", Vector) = (1920, 1080, 0, 1)
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 GetPixelSize()
            {
                return float2(1.0 / 640.0, 1.0 / 360.0);
            }
            ...
            float4 Frag(Varyings i) : SV_Target
            {
                ...
                float4 originalColor = GetMainColor(uv);

                Neighbours p = GetDepthNormalsNeighbours(uv, pixelSize);

                float sobel = Sobel(p, uv);
